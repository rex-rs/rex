...
- `perf`: Linux `perf` tools (`linux-perf` / `perf-tools-unstable` on Debian,
  `perf` on Fedora/Arch, `linuxPackages.perf` on NixOS).
- [FlameGraph](https://github.com/brendangregg/FlameGraph) scripts
  ...
  Note: `cargo install flamegraph` installs the
  [flamegraph-rs](https://github.com/flamegraph-rs/flamegraph) wrapper, which
  embeds the perl scripts inside its own binary and does **not** expose
  `flamegraph.pl` / `stackcollapse-perf.pl` on `PATH`, so this script can't
  reuse it.

  The script prints the resolved path and version on startup
  (e.g. `FlameGraph scripts: /nix/store/.../bin (version 2023-11-06)`)
  when the version can be inferred from the install location.
...
`-Cforce-frame-pointers=yes`, keeps `debug = "full"`, and sets
`split-debuginfo = "off"` so DWARF info lives in the binary, which is
required for dwarf-based kernel unwinding.
...
needed and re-chowns `perf.data` back to the invoking user so later analysis
works unprivileged.
